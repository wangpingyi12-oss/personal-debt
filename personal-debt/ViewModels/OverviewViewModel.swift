...
struct OverviewViewModel {
    static func makeMetrics(
        ...
        let lendingBalance = personalLendings.filter { $0.isValid }.reduce(0) { $0 + $1.remainingAmount }
        ...
        let overdueCredit = creditCards.flatMap(\.overdues).filter { $0.isActive && $0.isValid }.reduce(0) { $0 + $1.overdueAmount + $1.penaltyAmount }
        let overdueLoan = loans.flatMap(\.overdues).filter { $0.isActive && $0.isValid }.reduce(0) { $0 + $1.overdueAmount + $1.penaltyAmount }
        let overdue = overdueCredit + overdueLoan

        // Personal lending uses 已过约定日未还 (past-due) not overdue
        let pastDueAmount = personalLendings.filter { $0.isValid }.reduce(0) { $0 + $1.pastDueScheduledAmount }

        let creditLimitTotal = creditCards.reduce(0) { $0 + $1.creditLimit }
        let loanPrincipalTotal = loans.reduce(0) { $0 + $1.principal }
        let personalLendingPrincipalTotal = personalLendings.reduce(0) { $0 + $1.totalAmountDue }
        ...
        let todoCount = Int((monthDue > 0 ? 1 : 0) + (overdue > 0 ? 1 : 0) + (pastDueAmount > 0 ? 1 : 0))
